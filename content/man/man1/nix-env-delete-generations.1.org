#+TITLE: Manpages - nix-env-delete-generations.1
* Name
...
- *The special value *=old=
  ...
  *WARNING*
  ...
  one, but that is only true if the current generation is also the
  latest. Because one can roll back to a previous generation, it is
  possible to have generations newer than the current one. They will
  also be deleted.
...
collection effective. The is because profiles are also garbage
collection roots --- any *store object* reachable from a profile is
...
Delete the generations numbered 3, 4, and 8, so long as the current
active generation is not any of those.
...
Suppose =30= is the current generation, and we currently have
generations numbered =20= through =32=.

Then this command will delete generations =20= through =25=
(=<= 30 - 5=), and keep generations =26= through =31= (=> 30 - 5=).
