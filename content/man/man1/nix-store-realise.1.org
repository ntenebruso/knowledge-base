#+TITLE: Manpages - nix-store-realise.1
* Name
...
- If the path leads to a *store derivation*:
  ...
  - Try to fetch from *substituters* the *store objects* associated with
    ...
    - With *content-addressed derivations*: Determine the output paths
      to realise by querying content-addressed realisation entries in
      the *Nix database*.

  - For any store paths that cannot be substituted, produce the required
    store objects:
...
- Otherwise, and if the path is not already valid: Try to fetch the
  associated *store objects* in the path's *closure* from
  *substituters*.
...
* Examples
This operation is typically used to build *store derivation*s produced
by =nix-instantiate=:
