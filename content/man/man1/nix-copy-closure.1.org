#+TITLE: Manpages - nix-copy-closure.1
* Name
=nix-copy-closure= - copy store objects to or from a remote machine via
SSH
...
* Description
Given /paths/ from one machine, =nix-copy-closure= computes the
*closure* of those paths (i.e. all their dependencies in the Nix store),
and copies *store objects* in that closure to another machine via SSH.
It doesn't copy store objects that are already present on the other
machine.
...
Since =nix-copy-closure= calls =ssh=, you may need to authenticate with
the remote machine. In fact, you may be asked for authentication /twice/
...
- =--from=
  ...
  Copy the closure of /paths/ from the Nix store on the remote /machine/
  to the local machine's specified Nix store.
