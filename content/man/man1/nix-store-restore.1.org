#+TITLE: Manpages - nix-store-restore.1
* Name
...
** Description
The operation =--restore= unpacks a *Nix Archive (NAR)* to /path/, which
must not already exist. The archive is read from standard input.
