#+TITLE: Manpages - nix-store-dump.1
* Name
...
** Description
The operation =--dump= produces a *Nix archive* (NAR) file containing
the contents of the file system tree rooted at /path/. The archive is
...
store paths stored in Nix's database (see =nix-store --query= --hash)
