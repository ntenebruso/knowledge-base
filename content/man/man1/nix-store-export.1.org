#+TITLE: Manpages - nix-store-export.1
* Name
...
** Description
The operation =--export= writes a serialisation of the given *store
objects* to standard output in a format that can be imported into
another *Nix store* with =nix-store --import=.

#+begin_quote
*Warning*

This command /does not/ produce a *closure* of the specified store
paths. Trying to import a store object that refers to store paths not
...
archive* that /does not/ contain the set of *references* of a given
store path.
